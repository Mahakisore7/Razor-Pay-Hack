...
Scoped to exactly what T2.5's four starter rules (kill switch, domain
guards, consent, cost ceiling) consume. POLICY-ENGINE SS4's full contract
also carries `dnd_status`, `contact_history`, `promise_to_pay`,
`daily_spend`, `customer_timezone`, and `rate_limit_tokens` -- those arrive
with the rules that actually read them (R2, R5-R7, R9-R11), in whichever
phase builds them, not before. Notably absent for the same reason: a
`playbook: Playbook` field. None of the four rules here need more than the
...
__all__ = ["KillSwitchState", "PolicyContext"]
